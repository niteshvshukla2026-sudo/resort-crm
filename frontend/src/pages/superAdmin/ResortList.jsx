...
// small Maharashtra city list for datalist (keeps UX nice). This is not used as sample data.
const MAHA_CITIES = [
...
const ResortList = () => {
  ...
  // Load resorts from backend only
  const loadResorts = async () => {
    try {
      ...
      setError("Failed to load resorts from backend.");
      setResorts([]); // show empty so user sees backend needed
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFormChange = (e) => {
    ...
    setForm((p) => {
      ...
      if (name === "name") {
        // auto-generate code when name changes (only if user hasn't overridden)
    ...
    // clear field error for that field
  ...
  const handleDelete = async (r) => {
    ...
    try {
      ...
      await axios.delete(`${API_BASE}/api/resorts/${r._id || r.id}`).catch(() => {
  ...
  return (
    <div className="sa-page">
      ...
      {error && (
        <div style={{ fontSize: "0.85rem", marginBottom: 8, color: "#b91c1c", background: "rgba(250,204,210,0.3)", padding: 8, borderRadius: 8 }}>
          {error}
        </div>
      )}

      {/* Filters */}
      <div className="sa-card" style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        <label>
          Name
      ...
      {showForm && (
        <div className="sa-modal-backdrop" onClick={() => !saving && setShowForm(false)}>
          <div className="sa-modal" onClick={(e) => e.stopPropagation()} style={{ maxHeight: "85vh", overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <h3 style={{ marginBottom: 6 }}>{form._id ? "Edit Resort" : "New Resort"}</h3>
            <p className="sa-modal-sub" style={{ marginBottom: 10 }}>Add or update a property which will be available for transactions.</p>
            ...
            <form className="sa-modal-form" onSubmit={handleSubmit} style={{ overflow: "auto", paddingRight: 8 }}>
              ...
              <label>
                Code (auto)
                <input name="code" value={form.code} onChange={() => {}} placeholder="Auto-generated code" readOnly />
                <small style={{ display: "block", color: "#6b7280" }}>Auto-generated from name; backend will store this.</small>
              </label>
              ...
              <label>
                ...
                {fieldErrors.locationZone && <div className="field-error">{fieldErrors.locationZone}</div>}
              ...
              <label>
                ...
                {fieldErrors.ownerContact && <div className="field-error">{fieldErrors.ownerContact}</div>}
              </label>

              <label>
                Owner Email
                <input type="email" name="ownerEmail" value={form.ownerEmail} onChange={handleFormChange} placeholder="owner@example.com" />
                {fieldErrors.ownerEmail && <div className="field-error">{fieldErrors.ownerEmail}</div>}
              </label>
              ...
              <label>
                ...
                {fieldErrors.noOfKeys && <div className="field-error">{fieldErrors.noOfKeys}</div>}
              </label>
              ...
              <label>
                ...
                <input name="renewalDate" value={form.renewalDate} onChange={handleFormChange} placeholder="dd/mm/yyyy" />
                {fieldErrors.renewalDate && <div className="field-error">{fieldErrors.renewalDate}</div>}
              ...
              <div className="sa-modal-actions" style={{ marginTop: 12 }}>
                ...
                <button type="submit" className="sa-primary-button" disabled={saving}>{saving ? "Saving..." : form._id ? "Update Resort" : "Save Resort"}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="sa-modal-backdrop" onClick={() => setShowView(false)}>
          <div className="sa-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 800 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
              <div><strong>Address</strong><div style={{ color: "#374151" }}>{viewItem.address || "-"}</div></div>
              <div><strong>Location / Zone</strong><div style={{ color: "#374151" }}>{viewItem.locationZone || "-"}</div></div>
              <div><strong>Owner Name</strong><div style={{ color: "#374151" }}>{viewItem.ownerName || "-"}</div></div>
              <div><strong>Owner Contact</strong><div style={{ color: "#374151" }}>{viewItem.ownerContact || "-"}</div></div>
              <div><strong>Owner Email</strong><div style={{ color: "#374151" }}>{viewItem.ownerEmail || "-"}</div></div>
              <div><strong>No of Keys</strong><div style={{ color: "#374151" }}>{viewItem.noOfKeys ?? "-"}</div></div>
              <div><strong>Tie-up Category</strong><div style={{ color: "#374151" }}>{viewItem.tieUpCategory || "-"}</div></div>
              <div><strong>Quality Category</strong><div style={{ color: "#374151" }}>{viewItem.qualityCategory || "-"}</div></div>
              <div><strong>FSSAI Number</strong><div style={{ color: "#374151" }}>{viewItem.fssaiNumber || "-"}</div></div>
              <div><strong>FSSAI Status</strong><div style={{ color: "#374151" }}>{viewItem.fssaiStatus || "-"}</div></div>
              <div><strong>Renewal Date</strong><div style={{ color: "#374151" }}>{viewItem.renewalDate ? formatISOToDDMMYYYY(viewItem.renewalDate) : "-"}</div></div>
              <div><strong>Residential Address</strong><div style={{ color: "#374151" }}>{viewItem.ownerResdAddress || "-"}</div></div>
              <div><strong>Office Address</strong><div style={{ color: "#374151" }}>{viewItem.ownerOffAddress || "-"}</div></div>
            </div>
